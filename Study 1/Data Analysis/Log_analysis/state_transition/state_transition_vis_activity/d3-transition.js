...
function d3Transition() {
  ...
  function exports(selection) {
    ...
    selection.each(function(dd) {
      ...
      function updateNode() {
        // sort node
        var sortBy = ['collection', 'schematization', 'hypothesis', 'coordination']
        var node = container.select('.nodes').selectAll('.node')
          .data(d3.values(dd.nodes).sort(function(a, b) {
            return sortBy.indexOf(a.actionType) - sortBy.indexOf(b.actionType)
          }))
        ...
        node.select('text')
          ...
          .text(function(d) {
            if (d.actionType === 'collection') return 'e'
            return d.actionType[0]
          })
      }
